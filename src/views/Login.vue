<template>
  <v-container fluid>
    <v-layout row justify-center>
      <div id="googlesignin" @click="handleGoogleClickSignIn" v-if="services.google">
        <img src="/assets/images/btn_google_signin_dark_normal_web.png">
        <img src="/assets/images/btn_google_signin_dark_focus_web.png" class="img-top">
      </div>
      <div v-else>
        <img src="/assets/images/btn_google_signin_dark_disabled_web.png">
      </div>
    </v-layout>
    <v-layout row justify-center>
      <div v-if="services.facebook">
        <a href="" class="fb connect" @click="handleFacebookClickSignIn">Continuer avec Facebook</a>
      </div>
      <div v-else>
        <a href="" class="fb connect disabled" @click="handleFacebookClickSignIn">Continuer avec Facebook</a>
      </div>
    </v-layout>
    <v-layout row justify-center>
...
export default {
  ...
  methods: {
    ...
    async login () {
      await this.$store.dispatch('main/logPwd', { nom: this.nom, password: this.password })
      this.$router.push('/')
    },
    ...
    async handleFacebookClickSignIn (e) {
      e.preventDefault()
      window.FB.login(async response => {
        if (!response || !response.authResponse || !response.authResponse.accessToken) {
          throw new Error('Impossible de se connecter avec le service facebook')
        }
