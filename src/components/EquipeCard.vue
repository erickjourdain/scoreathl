      ...
      <v-card>
        ...
        <v-card-actions v-if="action">
          <v-layout row align-center justify-space-between="">
            <v-flex xs2>
              <v-btn fab dark small color="error" @click="$emit('del-team')" v-if="!equipe.statut">
...
export default {
  ...
  computed: {
    ...
    action () {
      let visible = true
      this.equipe.enfant.score.resultats.map(res => {
        if (res.resultat.statut) {
          visible = false
        }
      })
      this.equipe.adulte.score.resultats.map(res => {
        if (res.resultat.statut) {
          visible = false
        }
      })
